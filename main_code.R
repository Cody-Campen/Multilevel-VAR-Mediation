...
n_parameters = 5
n_outcomes = 1
treatment_effect_matrix = matrix(c(0, 0,   # X -> M_1 intercept
                                   0, 0,   # X -> M_2 intercept
                                   0, .2,  # X -> M_1 autoregression
                                   0, 0,   # X -> M_1 to M_2 crossregression
                                   0, 0,   # X -> M_2 to M_1 crossregression
                                   0, 0,   # X -> M_2 autoregression
                                   0, .1),  # X -> Z trans corr
                                 nrow = 7, ncol = n_treatments+1, byrow = T)
mediator_effect_matrix = matrix(c(.5,    # M_1 intercept -> Y
                                  0,     # M_2 intercept -> Y
                                  2.5,   # M_1 autoregression -> Y
                                  0,     # M_1 -> M_2 crossregression -> Y
                                  0,     # M_2 -> M_1 crossregression -> Y
                                  0,     # M_2 autoregression -> Y
                                  0),    # Z trans corr -> Y
                                nrow = 7, ncol = n_outcomes, byrow = T)
direct_effect = matrix(c(0,  # Y intercept
                         .5), # X -> Y
                       nrow = n_treatments + 1, ncol = n_outcomes, byrow = T)
parameter_matrix_covariance = diag(c(10, 10, .1, .1, .1, .1, .1))
...
parameter_matrix = data_info$parameter_matrix

# Structure the treatment
X = cbind(rep(1,n_people), as.matrix(dataset[dataset$time == 1,c("X1")], nrows=n_people))
...
first_missing = vector(length = n_people)
for(this_person in 1:n_people){
  first_missing[this_person] = ifelse(sum(is.na(M[this_person, 1, ]))==0, F, T)
...
first_observed = (1:n_people)[!first_missing]
first_missed = (1:n_people)[first_missing]
...
for(this_person in 1:n_people){
  ...
  if(n_seen[this_person] != 0){
    times_seen[this_person, 1:n_seen[this_person]] = which(is_missing[this_person, ] %in% 0)
  }
}
people_with_obs = which(n_seen > 0)
people_with_missing = which(n_miss > 0)
...
jags_data = list(X = X,
                 ...
                 people_with_obs = people_with_obs,
                 people_with_missing = people_with_missing,
                 n_treatments = n_treatments+1, # plus 1 because of the intercept term
                 n_parameters = n_parameters,
                 n_mediators = n_mediators,
                 n_outcomes = n_outcomes,
                 parameter_matrix.rate = diag(c(10, 10, .1, .1, .1, .1, .1)))
...
true_values = c(treatment_effect_matrix[1:2, ],     # effect of X on intercepts
                ...
                solve(parameter_matrix_covariance[1:n_parameters, 1:n_parameters]), # parameter precision matrix
                parameter_matrix_covariance[1:n_parameters, 1:n_parameters],        # parameter covariance matrix
