...
model {
  ...
  for(this_sub in 1:nsubs){
      ...
      Y.hat[1, this_sub] = M_fixed_effect[1:n_outcomes, 1:n_parameters] %*% parameter_matrix[1:n_parameters, this_sub] + direct_effect[1:n_outcomes, 1:n_treatments] %*% X[this_sub, 1:n_treatments]
  ...
  for(this_sub in 1:nsubs){
    ...
    for(j in 1:(n_mediators - 1)){ # column j
      for(i in (j+1):n_mediators){ # row i
        ...
        M_correlation_matrix[this_sub,i,j] = parameter_matrix[2*n_mediators + n_mediators^2 + (j-1)*n_mediators - (j*j - j)/2 + (i-j), this_sub]
      }
    }
