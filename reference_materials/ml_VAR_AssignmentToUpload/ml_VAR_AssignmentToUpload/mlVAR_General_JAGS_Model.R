...
model{
  ...
  for (pp in 1:P) {
    ...
    bmu[pp,1:nrCoeffmus] = muInter[pp,1:nrCoeffmus]
    for (dd in 1:D){
      bmu[pp,(nrCoeffmus+dd)] = muAR[pp,dd,dd]
    }
    ...
    for (dd in 1:D){
      ...
      # AR
      AR[pp,dd,dd] = b[pp, nrCoeffmus+dd]
  ...
  for (dd in 1:(D-1)) {
    for (dd2 in (dd+1):D) {
      v <- sd_noise[dd] * corr_ij[dd] * sd_noise[dd]
      Sigma_VAR[dd, dd2] <- v
      Sigma_VAR[dd2, dd] <- v
    }
  }
  ...
  for (dd in 1:D){
    ...
    sigmaAR[dd,dd] = sqrt(bcov[nrCoeffmus+dd,nrCoeffmus+dd])
  }
}# End of model
